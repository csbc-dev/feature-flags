...
このリポジトリ (`@csbc-dev/feature-flags`) は [`@wc-bindable/flags`](https://github.com/wc-bindable-protocol/wc-bindable-protocol/tree/main/packages/flags) を起点として、csbc-dev/arch のアーキテクチャ群の一員として再パッケージしたものです。設計思想を理解するうえで前提となる 2 つのドキュメントを以下にまとめます。
...
`@csbc-dev/feature-flags` は **ケース B2**: フラグ評価の決定（プロバイダ SDK 統合、ターゲティングルール評価、識別子管理、変更通知）はすべて `FlagsCore` (Core, `EventTarget`) がサーバ側で持ち、`<feature-flags>` (Shell, `HTMLElement`) はリモートセッション越しに評価結果を観測してローカルに公開するだけの**観測専用アダプタ**として振る舞う。Shell にはコマンド面（mutate 系メソッド）を持たせず、フラグの真実は常にサーバ側 Core に一元化される — 機密フラグや有償 SDK のキーをクライアントに露出させない構成になる。
